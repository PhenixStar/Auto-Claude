...
export function EnvConfigModal({
  ...
  const resolvedTitle = title ?? t('envConfig.defaultTitle');
  const resolvedDescription = description ?? t('envConfig.defaultDescription');
  ...
  const handleUseExistingProfile = async () => {
    ...
    try {
      ...
      if (!profile?.oauthToken) {
        setError(t('envConfig.selectedProfileNoToken'));
        setIsSaving(false);
        return;
      }
  ...
  const handleSave = async () => {
    if (!token.trim()) {
      setError(t('envConfig.tokenRequired'));
      return;
    }
            ...
            <div className="rounded-lg bg-success/10 border border-success/30 p-4 flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-success">
                  {t('envConfig.tokenConfiguredSuccess')}
                </p>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {t('envConfig.authenticatedAccounts', { count: claudeProfiles.length })}
                      </p>
                    </div>
                  </div>
                </div>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-foreground">
                            {profile.name}
                            {profile.isDefault && (
                              <span className="ml-2 text-xs text-muted-foreground">{t('envConfig.default')}</span>
                            )}
                          </p>
                          {profile.email && (
              ...
              <div className="space-y-3">
                <div className="rounded-lg bg-info/10 border border-info/30 p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <p className="text-sm text-foreground font-medium mb-1">
                        {claudeProfiles.length > 0 ? t('envConfig.orAuthenticateNewAccount') : t('envConfig.authenticateWithBrowser')}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {claudeProfiles.length > 0
                          ? t('envConfig.addNewAccountDescription')
                          : t('envConfig.browserAuthDescription')
                        }
                      </p>
                    </div>
                  </div>
              ...
              {showManualEntry && (
                <div className="space-y-3 pl-4 border-l-2 border-border">
                  ...
                  <div className="text-xs text-muted-foreground space-y-1">
                    <p className="font-medium text-foreground">{t('envConfig.manualStepsTitle')}</p>
                    <ol className="list-decimal list-inside space-y-1">
                      <li>{t('envConfig.manualStep1')}</li>
                      <li>
                        {t('envConfig.manualStep2Run')}{' '}
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t('envConfig.tokenSaveLocation')}{' '}
                      <code className="px-1 py-0.5 bg-muted rounded font-mono">
                        {sourcePath ? `${sourcePath}/.env` : t('envConfig.tokenSaveLocationDefault')}
                      </code>
                    </p>
                  </div>
                </div>
            ...
            {hasExistingToken && (
              <div className="rounded-lg bg-muted/50 p-3">
                <p className="text-sm text-muted-foreground">
                  {t('envConfig.existingTokenInfo')} {showManualEntry ? t('envConfig.existingTokenReplaceManual') : t('envConfig.existingTokenReplaceAuth')}
                </p>
              </div>
            )}
          </div>
